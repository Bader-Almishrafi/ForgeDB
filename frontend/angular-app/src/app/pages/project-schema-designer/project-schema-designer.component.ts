...
import { Observable, Subject, catchError, finalize, forkJoin, map, of, switchMap, take } from 'rxjs';
import {
  DatasetVersion,
  DesignModelResponse,
  DesignTable,
...
export class ProjectSchemaDesignerComponent implements OnInit, UnsavedChangesAware {
  ...
  copySql(): void {
    ...
    }).catch(() => this.feedback.set({ kind: 'error', title: 'Copy failed', message: 'Clipboard permission was denied.' }));
  }
  ...
  columnError(columnId: number, field = 'name'): string {
  ...
  focusIssue(issue: ValidationIssue): void {
    if (issue.tableId) this.selectedTableId.set(issue.tableId);
  }
  ...
  continueToExport(): void {
    ...
    void this.router.navigate(
      ['/projects', this.projectId, 'export-deploy'],
  ...
  protectBrowserUnload(event: BeforeUnloadEvent): void {
    if (!this.allowNavigation && this.dirty()) {
  ...
  private applyWorkspace(workspace: SchemaWorkspace): void {
    ...
    this.applyDatasetQuery(workspace.workflow);
    this.applyDesign(workspace.design);
    this.loadSqlPreview();
  }

  private applyDatasetQuery(workflow: ProjectWorkflow): void {
    const requested = Number(this.route.snapshot.queryParamMap.get('datasetId'));
    const datasetId = Number.isInteger(requested) && requested > 0
      && workflow.datasets.some((dataset) => dataset.datasetId === requested)
      ? requested
      : null;
